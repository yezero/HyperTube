...
function getCast(req, res) {
    var url = "https://api.themoviedb.org/3/search/person?" +
        ...
        "query="+req.query.cast+"&page=1&include_adult=false";
    var castIds = "-";
...
function getKeyword(req, res) {
    var url = "https://api.themoviedb.org/3/search/keyword?" +
        "api_key=88fb971d6edf2b3de88914b0157a7cca" +
        "&query="+req.query.word+"&page=1";
    request(url, function (err, response, body) {
        if (err || response.statusCode != 200){
            return res.status(500).json({
                title: "An error occurred",
...
function getSearch(req, res) {
    var url = "https://api.themoviedb.org/3/discover/movie?" +
        "api_key=88fb971d6edf2b3de88914b0157a7cca&language=en-US&include_adult=true";

    url = url + "&page=" + req.query.page;
    if (req.query.cast != '-')
        url = url + "&with_cast=" + req.query.cast;
    if (req.query.word != '-')
        url = url + "&with_keywords=" + req.query.cast;
    if (req.query.from_year != '-')
        url = url + "&primary_release_date.gte=" + req.query.from_year+"-01-01";
    if (req.query.to_year != '-')
        url = url + "&primary_release_date.lte=" + req.query.to_year+"-01-01";
    if (req.query.rate_from != '-')
        url = url + "&vote_average.gte=" + req.query.rate_from;
    if (req.query.rate_to != '-')
        url = url + "&vote_average.lte=" + req.query.rate_to;
    if (req.query.sort != '-')
...
function arrange(results, type, title) {
    var data;
    var group =[];
